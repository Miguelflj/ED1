...
void RemoveBin(arvBin **raiz, int n){
    arvBin *aux;

    aux = *raiz;

    if(aux == NULL){
        puts("Nó não existe na arvore");
        return;
    ...
    if(n < aux->valor){
        ...
        // se nao é menor nem maior, é o no que eu busco
 ...
 * 1 caso =  o no é folha
 * somente Remover
 * 2 caso = o noh possui so o filho esquerdo
 * coloca o filho no lugar
 * 3 caso = o noh possui so o filho direito
 * coloca o filho no lugar
 * 
 * 4 caso = o no possui dois filhos
 * entao escolheo o maior no do ramo esquerdo para substituir este no e faz a chamada recursiva
 * if( (*arq)->esq != NULL && (&arv)->dir != NULL){
 * 
 *  no = (*arv);
 *  int maior;
 * maior = buscaMaior( (*arv)->esq );
 * (*arv)->valor = maior;
 * removeArvBin ( &(*arv)->esq, maior).
 * return no;
 * */
